...
//           <p className="text-slate-500 mt-1 text-sm sm:text-lg font-medium">Ready for your Healthcare Checkup?</p>
//         </div>
...
const Dashboard = () => {
  ...
  const viewSession = (session) => {
    if (session.status === "completed" || session.status === "in-progress") {
      ...
      toast.info("Your AI session is being prepared...");
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-12 space-y-8 sm:space-y-12 animate-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6 sm:pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-sm sm:text-lg font-medium">
            Your AI Healthcare Assistant is ready to help.
          </p>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200 border border-slate-100 overflow-hidden">
        ...
        <form onSubmit={onSubmit} className="p-8 sm:p-12">
          ...
          <div className="max-w-2xl mx-auto text-center space-y-8">
            ...
            <div className="w-20 h-20 mx-auto rounded-3xl bg-teal-100 flex items-center justify-center">
              <svg className="w-10 h-10 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <rect x="4" y="7" width="16" height="12" rx="2" strokeWidth="2.5"/>
            ...
            <div>
              <h3 className="text-3xl font-black text-slate-900">
                Talk with your AI Healthcare Assistant
              </h3>

              <p className="mt-4 text-slate-500 leading-7">
                Describe your symptoms, ask health-related questions,
                understand possible conditions, and receive personalized
                healthcare guidance through a natural conversation.
              </p>
            </div>
            ...
            <button
              ...
              className={`h-12 px-8 rounded-xl font-bold text-white transition ${
                isGenerating
                  ? "bg-slate-300"
                  : "bg-teal-600 hover:bg-teal-700"
              }`}
            >
              {isGenerating ? "Starting..." : "Start Session"}
            </button>
      ...
      <div className="space-y-6 pb-20 sm:pb-0">
        ...
        <h2 className="text-xl sm:text-2xl font-black text-slate-800 flex items-center px-2">
          <span className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center mr-3">
            🩺
          </span>
          Session History
        </h2>
        ...
        {isLoading && sessions.length === 0 ? (
          ...
          <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] py-20 text-center">
            ...
            <p className="mt-2 text-slate-500">
              Start your first conversation with the AI Healthcare Assistant.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
