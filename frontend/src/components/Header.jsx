  ...
  return (
    <header className="bg-slate-900/95 backdrop-blur-md text-white shadow-2xl sticky top-0 z-50 border-b border-slate-700/50 py-2">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items center">
        <Link to={getDashboardPath()} className="flex items-center space-x-2 group shrink-0">
          <div className="bg-teal-500 p-1.5 rounded-lg group-hover:rotate-12 transition-transform duration-300">
            <svg className="w-5 h-5 sm:w-6 sm:h-6 text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <rect x="4" y="7" width="16" height="12" rx="2" ry="2" strokeWidth="2.5"/>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 4h6v3M12 10v6M9 13h6"/>
            </svg>

          </div> 
          <span className=" text-lg sm:text-xl font-black tracking-tighter uppercase text-white group-hover:text-teal-400 transition-colors">Doc<span className="text-teal-500">tor</span><span className="hidden sm:inline">G</span></span>
        </Link>
        <nav className="hidden md:flex items-center space-x-6">
          {user ? (<>
            <Link to={getDashboardPath()} className={`text-sm font-bold uppercase tracking-widest transition-all ${isActive(getDashboardPath()) ? 'text-teal-400 border-b-2 border-teal-500' : 'text-slate-400 hover:text-white border-b-2 border-transparent'}`}>Dashboard</Link>
            {user.role?.toLowerCase() !== 'admin' && user.role?.toLowerCase() !== 'doctor' && (
              <Link to="/appointment" className={`text-sm font-bold uppercase tracking-widest transition-all ${isActive('/appointment') ? 'text-teal-400 border-b-2 border-teal-500' : 'text-slate-400 hover:text-white border-b-2 border-transparent'}`}>Appointment</Link>
            )}
            ...
            <div className="flex items-center space-x-2 bg-slate-800/50 px-4 py-1.5 rounded-full border border-slate-700">
              ...
              <span className="text-xs font-bold text-slate-300 uppercase">{user?.name ? user.name.split(' ')[0] : 'User'}</span>
            </div>
          ...
          <div className="px-6 py-8 space-y-4">
            {user ? (
              <>
                ...
                <Link to={getDashboardPath()} onClick={() => setIsMenuOpen(false)} className={`block py-4 text-xl font-black uppercase tracking-widest border-b border-slate-800 ${isActive(getDashboardPath()) ? 'text-teal-400' : 'text-slate-400'}`}>Dashboard</Link>
                {user.role?.toLowerCase() !== 'admin' && user.role?.toLowerCase() !== 'doctor' && (
                  <Link to="/appointment" onClick={() => setIsMenuOpen(false)} className={`block py-4 text-xl font-black uppercase tracking-widest border-b border-slate-800 ${isActive('/appointment') ? 'text-teal-400' : 'text-slate-400'}`}>Appointment</Link>
                )}
